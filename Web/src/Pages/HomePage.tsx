...
const HomePage = () => {
  ...
  return (
    <div className="bg-soft-bg text-brand-text font-sans min-h-[calc(100vh-64px)]">
      {/* Hero Section */}
      <main
        className={`max-w-7xl mx-auto px-6 ${isMobile ? "pt-10 pb-20" : "pt-20 pb-32"} text-center`}
      >
        ...
        <h2
          className={`${isMobile ? "text-4xl" : "text-5xl md:text-7xl"} font-extrabold mb-8 tracking-tight leading-tight text-brand-text`}
        >
          Your trips, <span className="text-brand-primary">summarized</span>.
          <br />
          Your preferences,{" "}
          <span className="text-brand-primary">understood</span>.
        </h2>
        <p
          className={`${isMobile ? "text-base" : "text-lg md:text-xl"} text-brand-text-muted max-w-2xl mx-auto mb-12 leading-relaxed`}
        >
        ...
        {/* Feature Preview */}
        <div
          className={`mt-24 grid ${isMobile ? "grid-cols-1 gap-6" : "md:grid-cols-3 gap-8"}`}
        >
          {[
            {
